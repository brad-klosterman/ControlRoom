...
export function createChainedFunction<T extends ChainableFunction>(
  ...funcs: (T | false)[]
): ChainableFunction {
  return (...args: any[]) => {
    (funcs.filter(Boolean) as T[]).forEach(x => {
      x(...args);
    });
  };
}

export function useChainedEventCallback<T extends ChainableFunction>(
  ...callbacks: (T | false)[]
): ChainableFunction {
