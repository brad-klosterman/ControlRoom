...
export class FocusManager {
  ...
  constructor(
    scope: DomScope,
    { isVirtual = false, keyProp }: FocusManagerOptions = {},
  ) {
    this.scope = scope;
  ...
  private focusElement(
    element?: HTMLElement,
    { onFocus, onNotFound }: FocusOptions = {},
  ) {
    if (!isNil(element)) {
      if (this.isVirtual) {
      ...
      } else if (isFunction(element.focus)) {
        element.focus();
  ...
  focusFirst({ canFocus, ...options }: FocusOptions = {}) {
    ...
    if (elements?.length && elements.length > 0) {
      if (isNil(canFocus)) {
        ...
        } while (!isNil(target) && !canFocus(target));
      }
    }
  ...
  focusKey(key: string, options?: FocusOptions) {
    ...
    if (isNil(this.keyProp)) {
      throw new Error(
        '"focusKey" cannot be called without providing a `keyProp` to the FocusManager.',
      );
    }

    return this.focusElement(
      elements?.find(x => x.getAttribute(this.keyProp!) === key?.toString()),
      options,
    );
  }
  ...
  search(query: string, options?: FocusOptions) {
    ...
    return this.focusElement(
      elements?.find(x => x.textContent?.toLowerCase().startsWith(query)),
      options,
    );
  }
  ...
  getActiveKey() {
    if (isNil(this.keyProp)) {
      throw new Error(
        '"getActiveKey" cannot be called without providing a `keyProp` to the FocusManager.',
      );
    }

    const activeElement = this.getActiveElement();

    return !isNil(activeElement)
      ? activeElement.getAttribute(this.keyProp)
      : null;
  }
}
