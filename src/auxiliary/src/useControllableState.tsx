...
function ensureControlledStateHaveNotChanged<T>(
  controlledValue: T,
  isControlled: boolean,
) {
  if (
    (isControlled && isUndefined(controlledValue)) ||
    (!isControlled && !isUndefined(controlledValue))
  ) {
    throw new Error(
      'useControllableState - A controllable state value cannot switch between controlled and uncontrolled. Did you inadvertently set a default value (defaultProps) for your controlled prop?',
    );
  }
}

function useComputeInitialState<T>(
  ...
  if (isUndefined(controlledValue)) {
    ...
    state = (!isUndefined(initialValue) ? initialValue : defaultValue) as T;
  } else {
...
function computeSubsequentState<T>(
  controlledValue: T,
  currentState: T,
  isControlled: boolean | null,
) {
...
export interface OnChangeContext {
  isControlled: boolean | null; // BK EDIT
  isInitial: boolean | null; // BK EDIT
}
...
export function useControllableState<T>(
  ...
  initialValue: T | undefined,
  defaultValue: T | undefined,
  { onChange }: ControllableStateOptions<T> = {},
): [T, (maybeState: T) => void, boolean | null] {
  ...
  let {
    ...
    isInitialState,
    state: initialState,
  } = useComputeInitialState(controlledValue, initialValue, defaultValue);

  const [isControlledRef] = useRefState(isControlledProp);

  const transformState = useCallback(
    (newState: T, context: Omit<OnChangeContext, 'isControlled'>) => {
      ...
      return !isUndefined(transformedState) ? transformedState : newState;
    },
    [onChange, isControlledRef],
  );

  if (isInitialState) {
    initialState = transformState(initialState as T, { isInitial: true });
  }

  /* 
    Using a ref instead of useState because when in controlled mode the consumer must already have is own state management code andusing 
    useState here would cause 2 rerender when the controlled value update.
  */
  const [stateRef, setState] = useRefState(initialState);

  if (!isInitialState) {
    const { hasChanged, newState } = computeSubsequentState(
      controlledValue,
      stateRef.current,
    ...
    if (hasChanged) {
      setState(transformState(newState as T, { isInitial: false }));
    }
  }
   ...
   */
  const setUncontrolledState = useCallback(
    (maybeState: T) => {
  ...
  return [stateRef.current as T, setUncontrolledState, isControlledProp];
}
