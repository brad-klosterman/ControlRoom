...
import {
  CREATE_DISCOUNT_PRICE_DOCUMENT,
  CREATE_DISCOUNT_PRICE_INPUT,
  DELETE_DISCOUNT_PRICE_DOCUMENT,
  DELETE_DISCOUNT_TEMPLATE_DOCUMENT,
  DISCOUNT_PRICE_CORE_FRAGMENT,
  DISCOUNT_PRICE_CORE_FRAGMENT_DOC,
  DISCOUNT_TEMPLATE_CORE_FRAGMENT,
  UPDATE_DISCOUNT_PRICE_DOCUMENT,
  UPDATE_DISCOUNT_TEMPLATE_DOCUMENT,
...
const ExpandedContent = (props: {
  discount: DISCOUNT_TEMPLATE_CORE_FRAGMENT;
  refetch(): void;
   ...
   * DISCOUNT FORM
   */
  const discount_form = useForm<UPDATE_DISCOUNT_TEMPLATE_MUTATION_VARIABLES>({
    mode: 'onSubmit',
    reValidateMode: 'onChange',
  });

  useEffect(() => {
    discount_form.reset(discount);
  }, [discount.id]);
   ...
   */
  const price_matrix =
    getFragment(DISCOUNT_PRICE_CORE_FRAGMENT_DOC, discount.price_matrix) || [];

  const price_matrix_form = useForm<{
    prices: DISCOUNT_PRICE_CORE_FRAGMENT[];
  }>({
    mode: 'onSubmit',
    reValidateMode: 'onChange',
  });
   ...
   * NEW PRICE FORM
   */
  const new_price_form = useForm<CREATE_DISCOUNT_PRICE_INPUT>({
    mode: 'onSubmit',
    reValidateMode: 'onChange',
  });
   ...
   * MUTATIONS
   */
  const [update_discount_template] = useMutation(
    UPDATE_DISCOUNT_TEMPLATE_DOCUMENT,
  );
  const [delete_discount_template] = useMutation(
    DELETE_DISCOUNT_TEMPLATE_DOCUMENT,
  );
  const [create_discount_price] = useMutation(CREATE_DISCOUNT_PRICE_DOCUMENT);
  const [update_discount_price] = useMutation(UPDATE_DISCOUNT_PRICE_DOCUMENT);
  const [delete_discount_price] = useMutation(DELETE_DISCOUNT_PRICE_DOCUMENT);

  const onUpdateDiscountTemplate: SubmitHandler<
    UPDATE_DISCOUNT_TEMPLATE_MUTATION_VARIABLES
  > = async form_values => {
    modal.open({
      title: 'UPDATING DISCOUNT TEMPLATE',
      subtitle: `Do you want to update ${discount.name} discount template?`,
      actions: [
        { type: 'cancel' },
      ...
      async onConfirm() {
        try {
          await update_discount_template({
            variables: {
              id: discount.id,
          ...
          toast.successAlert('UPDATED DISCOUNT TEMPLATE');
          props.refetch();
          modal.close();
        } catch (error) {
          toast.errorAlert('COULD NOT UPDATED DISCOUNT TEMPLATE');
        }
      },
    });
  };
  ...
  const openDeleteDiscountTemplate = async () => {
    modal.open({
      title: 'DELETING DISCOUNT TEMPLATE',
      subtitle: `Do you want to delete ${discount.name}?`,
      actions: [
        { type: 'cancel' },
      ...
      async onConfirm() {
        try {
          await delete_discount_template({
            variables: {
              id: discount.id,
            },
          });

          toast.successAlert('DELETED DISCOUNT TEMPLATE');
          props.refetch();
          modal.close();
        } catch (error) {
          toast.errorAlert('COULD NOT DELETE DISCOUNT TEMPLATE');
        }
      },
    });
  };
      ...
      <Expanded.Inner>
        <Grid repeat={20} spacing="large">
          ...
          <Grid.Cell x={[0, 20]} y={[4, 5]}>
            <Actions>
              <Button onClick={openDeleteDiscountTemplate} variant="delete">
                DELETE
              </Button>
              <Save
                disabled={!discount_form.formState.isDirty}
                onClick={discount_form.handleSubmit(onUpdateDiscountTemplate)}
              >
                SAVE
              </Save>
            </Actions>
