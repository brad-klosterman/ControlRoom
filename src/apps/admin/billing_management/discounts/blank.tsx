...
import {
  ...
  CREATE_DISCOUNT_TEMPLATE_DOCUMENT,
...
const NewTemplate = (props: {
  ...
  const prices = useFieldArray({
    ...
    name: 'discount_pricings',
  });

  /**
   * NEW PRICE FORM
   */
  const price_form = useForm<CREATE_DISCOUNT_PRICE_INPUT>({
    mode: 'onSubmit',
    reValidateMode: 'onChange',
  });

  const [create_discount_template] = useMutation(
    CREATE_DISCOUNT_TEMPLATE_DOCUMENT,
  );
  ...
  > = async variables => {
    create_discount_template({
      ...
      if (res.data?.create_discount_template.success) {
                ...
                <Fragment key={p.id}>
                  ...
                  <FormExpander.Content>
                    <ControlledMaskedInput
                      ...
                      mask="999999999999"
                      name={`discount_pricings.${i}.price_excl_tax`}
                    />
